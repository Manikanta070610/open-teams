...
- Schema lives in `db/` and must apply cleanly in numeric order: `psql -f 01..08`.
...
- Fresh-build command (from workspace root):
  `dropdb --if-exists office_mgmt_final && createdb office_mgmt_final && for f in db/01_core.sql db/02_projects.sql db/03_hr.sql db/04_collab_chat.sql db/05_triggers.sql db/06_indexes.sql db/07_seed.sql db/08_dept_governance.sql; do psql -d office_mgmt_final -v ON_ERROR_STOP=1 -q -f "$f"; done`
- Expected seed counts: `levels 6 / depts 6 / emps 7 / projs 1 / tasks 1`.
